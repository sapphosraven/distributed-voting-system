...
# Configure more detailed logging for clock sync
...
def log_state():
    """Debug function to log current sync state"""
    state = {
        "is_leader": node_state.is_leader if node_state else None,
        "initial_sync_done": initial_sync_done,
        "last_sync_time": last_sync_time,
        "current_offset": current_offset,
        "sync_history_size": len(sync_history),
        "sync_history": sync_history[:3]  # Show up to 3 entries
    }
    logger.debug(f"CLOCK SYNC STATE: {state}")
...
def init_clock_sync(comm, state):
    ...
    # Always send an immediate sync request for followers
    if communicator and not node_state.is_leader:
        logger.info("Scheduling immediate sync request at initialization")
...
async def request_immediate_sync():
    ...
    if not communicator or node_state.is_leader:
        logger.debug("Skipping sync request - node is leader or communicator not ready")
        return
    
    sync_request_count += 1
    logger.info(f"[SYNC REQUEST] Requesting immediate time synchronization from leader (attempt {sync_request_count})")
    
    try:
        # Wait a brief moment to ensure communicator is ready
        await asyncio.sleep(1)
        
        # Send urgent sync request to leader
        logger.debug(f"Sending sync_request message with ID: sync-req-{sync_request_count}")
        ...
        # Schedule another request if we don't get a response soon
...
async def leader_time_sync_loop():
    ...
    logger.info("Sending initial time sync broadcast")
    await send_time_sync(True)
    
    # Then continue with periodic broadcasts
    while True:
        if node_state and node_state.is_leader:
            try:
                await send_time_sync()
            except Exception as e:
                logger.error(f"Error in leader time sync: {e}", exc_info=True)
...
async def send_time_sync(is_initial=False):
    ...
    # Add type field for easier message processing
    message_data = {
    ...
    logger.debug(f"[LEADER] Sending {sync_type} time sync: {current_time}")
    
    # Broadcast to all followers
    communicator.send_message("time_sync", "broadcast", message_data)
        ...
        if node_state and not node_state.is_leader:
            try:
                ...
                if initial_sync_done and last_sync_time > 0:
                    ...
                    # Master time should be ahead by time_since_sync
                    ...
                    # Log drift info
                    logger.debug(f"DRIFT: current={drift:.6f}s, median={median_drift:.6f}s, time_since_sync={time_since_sync:.2f}s")
                    
                    # Apply correction based on drift magnitude
                    if abs(median_drift) > 10.0:  # Extreme drift
                        correction = median_drift * 0.95  # 95% correction
                        logger.warning(f"Extreme clock drift detected: {median_drift:.4f}s")
                        node_state.system_time = local_time + correction
                        logger.info(f"Applied aggressive correction: {correction:.4f}s")
                        
                        # Request immediate resync for extreme drift
                        asyncio.create_task(request_immediate_sync())
                    
                    elif abs(median_drift) > 5.0:  # Severe drift
                        ...
                        node_state.system_time = local_time + correction
                        logger.debug(f"Applied minor correction: {correction:.4f}s")
                
                else:
                    # If we've never synchronized, request it
                    if not initial_sync_done and time.time() % 5 < 0.1:  # Log occasionally
                        logger.warning("Never synchronized with leader, requesting sync")
                        asyncio.create_task(request_immediate_sync())
...
def handle_time_sync_message(message):
    ...
    logger.debug(f"[TIME SYNC] Received message type={message_type}, sender={sender}")
    
    # Skip if we're the leader - leader sets time, doesn't sync to it
    if node_state and node_state.is_leader:
        # But respond to sync requests if we're the leader
        if message_type == "sync_request":
            logger.info(f"[LEADER] Received sync request from {sender}, sending immediate response")
    ...
    if message_type == "broadcast" or (isinstance(data, dict) and "system_time" in data):
        try:
            ...
            if system_time <= 0:
                logger.warning(f"Invalid system time in sync message: {system_time}")
                return
            ...
            logger.info(f"Time sync from {sender}: leader_time={system_time:.6f}, local_time={local_time:.6f}, drift={drift:.6f}s")
...
def get_drift_info():
    ...
    current_time = time.time()
    sync_age = current_time - last_sync_time if last_sync_time > 0 else float('inf')
    
    logger.debug(f"Drift info: offset={current_offset:.6f}, sync_age={sync_age:.2f}s, synced={initial_sync_done and sync_age < 30.0}")
